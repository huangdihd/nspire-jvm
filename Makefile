...
build/nspire-jvm: $(HOST_SOURCES) src/vm.h src/context.h src/threads.inc src/unsafe.inc src/loader.inc src/identifiers.inc src/indy.inc src/format.inc src/reflection.inc src/xml.inc src/expat_config.h $(wildcard vendor/expat/lib/*.h) vendor/miniz.h Makefile
	mkdir -p build
	$(CC) $(COMMON) $(WARN) $(CFLAGS) $(HOST_SOURCES) -lm -o $@

ndless: dist/nspire-jvm.tns
build/ndless/%.o: %.c src/vm.h src/context.h src/threads.inc src/unsafe.inc src/loader.inc src/identifiers.inc src/indy.inc src/format.inc src/reflection.inc src/xml.inc src/expat_config.h $(wildcard vendor/expat/lib/*.h) Makefile
	mkdir -p $(dir $@)
