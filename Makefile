...
build/nspire-jvm: $(SOURCES) src/vm.h src/context.h src/threads.inc src/unsafe.inc src/loader.inc src/identifiers.inc src/indy.inc vendor/miniz.h Makefile
	mkdir -p build
	$(CC) $(COMMON) $(WARN) $(CFLAGS) $(SOURCES) -lm -o $@

ndless: dist/nspire-jvm.tns
build/ndless/%.o: %.c src/vm.h src/context.h src/threads.inc src/unsafe.inc src/loader.inc src/identifiers.inc src/indy.inc Makefile
	mkdir -p $(dir $@)
