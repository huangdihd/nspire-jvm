...
> **已进入原始 Xinbot.main 的检查点。** 已加入 record 生成方法及整数解析；原始应用通过版本初始化后，在 LangManager 使用的 `Map.of` 处停止。完整启动、联网和计算器实机执行仍未完成。范围见 [CHECKPOINT.md](CHECKPOINT.md)、[RECORD-SUPPORT.md](RECORD-SUPPORT.md)。
...
- Record 基类与 Class.isRecord、javac 生成的 ObjectMethods 字段比较/哈希/文本；原始 FloatingDecimal 负责浮点字段转换，真实回调、GC 和异常已做 Java 17 对照，见 `RECORD-SUPPORT.md`。
- Integer.parseInt/Long.parseLong 的十进制与显式进制重载，以及整数按进制转字符串；包含 Unicode 数字、符号、溢出和异常路径。

- Method/Constructor 参数反射：真实名称、标志、声明方法、原始类型、直接注解、缓存及防御性数组；泛型与类型注解边界见 `PARAMETER-SUPPORT.md`。
- 实际 JAR Manifest 包元数据读取，包专属覆盖、UTF-8 折行、缺失值及类定义顺序；同时支持旧式 high-byte String 构造器，见 `PACKAGE-METADATA-SUPPORT.md`。
...
python3 tools/test-parameters.py --java /path/to/java8/bin/java --vm build/nspire-jvm
python3 tools/test-package-metadata.py --java /path/to/java8/bin/java --vm build/nspire-jvm
python3 tools/test-records.py --java /path/to/java17/bin/java --vm build/nspire-jvm --xinbot /path/to/xinbot.jar
...
源码、固定版本和授权位于 `runtime/openjdk8/`，共 451 个未修改的上游源文件；`vendor/openjdk8-nio/` 另保存生成模板、工具和 80 个生成源码。`vendor/openjdk8-time/` 保存时区数据和两份原始加载器，修改后的加载器位于 `runtime/nspire/`，仍保留上游许可证。
...
新增八项 record/整数转换检查通过普通及 ASan/UBSan/泄漏检测构建，包括原始 Xinbot Version 组件、312 组浮点文本以及错误元数据检查；见 `RECORD-RESULTS.txt`。原始应用已进入 main，但完整启动仍失败于 `Map.of`。
...
- 通用 `invokedynamic`、对象流序列化、MethodHandle API、动态常量和完整反射（字段值访问、RecordComponent、重复参数注解、类型使用位置的注解、泛型等）。方法反射和 lambda 的支持边界分别见 `METHOD-SUPPORT.md`、`LAMBDA-SUPPORT.md`；字符串拼接暂不支持 float/double 的 Java 格式化。
...
```text
VM error: runtime method not implemented: java/lang/Object.of(Ljava/lang/Object;Ljava/lang/Object;Ljava/lang/Object;Ljava/lang/Object;Ljava/lang/Object;Ljava/lang/Object;Ljava/lang/Object;Ljava/lang/Object;)Ljava/util/Map;
  at xin/bbtt/mcbot/LangManager.<clinit>()V pc=35
  at xin/bbtt/mcbot/Xinbot.main([Ljava/lang/String;)V pc=2
```

真实 SLF4J 服务发现已找到 Logback 提供者，读取版本属性、生成状态消息，并反射创建配置器。
当前已创建并使用配置事件的 lambda，解析原始 XML，并使用真实 Stream.noneMatch 完成路径匹配。实际注解已用于选择配置处理阶段，Pattern 编译、环境变量替换和 AtomicBoolean 初始化已通过。现在成功创建 Xinbot 自身的 JLineConsoleAppender，BeanDescriptionFactory 已完成继承方法发现和类型查询。时间库、TZDB 和可序列化 lambda 协议现已让原始日期转换器完成初始化。实际 ConsoleAppender 随后按原始配置启用 Jansi，已创建描述符输出流并设置清理线程优先级，库名映射、临时目录和 NIO 复制已让它实际提取出 18,976 字节的库文件，其内容与原始 JAR 一致；现在 File.deleteOnExit 已登记退出清理，Jansi 捕获真实的 JNI 加载失败，String.join 已构造错误消息；Logback 的参数反射查询已通过，Xinbot 随后读取实际包版本、解析并创建版本 record，现已进入 Xinbot.main，在 LangManager 初始化使用的 Map.of 处停止。错误中的 Object.of 是现有方法查找回退产生的名称；原始字节码的调用目标确实是 java/util/Map.of。完整堆栈见 `XINBOT-RUN.txt`。
...
`src/records.inc` 执行 javac record 的字段 bootstrap；`src/parameters.inc` 连接真实参数元数据和原始 Parameter 实现。
`src/methods.inc` 实现方法发现与调用及包元数据读取，`src/boxing.inc` 补充基本类型包装类；内建类的反射声明数据位于 `vendor/openjdk8-api/`，声明不代表已实现该 API。
