...
/**
 * Ndless/host adaptation of OpenJDK UnixPath. Lexical path algorithms are
 * unchanged. Platform operations use NspireNative/the provider; the encoder
 * cache uses an unqueued weak reference. Original: vendor/openjdk8-path.
 */

class NspirePath
    extends AbstractPath
{
    private static ThreadLocal<WeakReference<CharsetEncoder>> encoder =
        new ThreadLocal<WeakReference<CharsetEncoder>>();

    // FIXME - eliminate this reference to reduce space
    private final NspireFileSystem fs;
    ...
    NspirePath(NspireFileSystem fs, byte[] path) {
        this.fs = fs;
        this.path = path;
    }

    NspirePath(NspireFileSystem fs, String input) {
    ...
    private static byte[] encode(NspireFileSystem fs, String input) {
        WeakReference<CharsetEncoder> ref = encoder.get();
        CharsetEncoder ce = (ref != null) ? ref.get() : null;
        if (ce == null) {
            ce = Util.jnuEncoding().newEncoder()
                .onMalformedInput(CodingErrorAction.REPORT)
                .onUnmappableCharacter(CodingErrorAction.REPORT);
            encoder.set(new WeakReference<CharsetEncoder>(ce));
        }
    ...
    // Checks that the given file is a NspirePath
    static NspirePath toNspirePath(Path obj) {
        if (obj == null)
            throw new NullPointerException();
        if (!(obj instanceof NspirePath))
            throw new ProviderMismatchException();
        return (NspirePath)obj;
    }
    ...
    private NspirePath emptyPath() {
        return new NspirePath(getFileSystem(), new byte[0]);
    }

    @Override
    public NspireFileSystem getFileSystem() {
        return fs;
    }

    @Override
    public NspirePath getRoot() {
    ...
    public NspirePath getFileName() {
        ...
        return new NspirePath(getFileSystem(), result);
    }

    @Override
    public NspirePath getParent() {
        initOffsets();
    ...
    @Override
    public NspirePath getName(int index) {
        initOffsets();
        if (index < 0)
    ...
    @Override
    public NspirePath subpath(int beginIndex, int endIndex) {
        initOffsets();
    ...
    @Override
    public NspirePath resolve(Path obj) {
        byte[] other = toNspirePath(obj).path;
    ...
    NspirePath resolve(byte[] other) {
        return resolve(new NspirePath(getFileSystem(), other));
    }

    @Override
    public NspirePath relativize(Path obj) {
        NspirePath other = toNspirePath(obj);
        ...
        if (i < cn) {
            ...
            NspirePath remainder = other.subpath(i, cn);
            if (dotdots == 0)
                return remainder;
    ...
    public boolean startsWith(Path other) {
        if (!(Objects.requireNonNull(other) instanceof NspirePath))
            return false;
        NspirePath that = (NspirePath)other;
    ...
    public int compareTo(Path other) {
        ...
        int len2 = ((NspirePath) other).path.length;

        int n = Math.min(len1, len2);
        byte v1[] = path;
        byte v2[] = ((NspirePath) other).path;
    ...
    public boolean equals(Object ob) {
        if ((ob != null) && (ob instanceof NspirePath)) {
    ...
    @Override
    public NspirePath toAbsolutePath() {
        if (isAbsolute()) {
            return this;
        }
        ...
        return new NspirePath(getFileSystem(),
    ...
    public Path toRealPath(LinkOption... options) throws IOException {
        ...
        NspirePath absolute = toAbsolutePath();

        // if resolving links then use realpath
        if (Util.followLinks(options)) {
            return new NspirePath(fs, NspireNative.realPath(absolute.toString(), toString()));
        }

        // if not resolving links then eliminate "." and also ".."
        // where the previous element is not a link.
        NspirePath result = fs.rootDirectory();
        for (int i=0; i<absolute.getNameCount(); i++) {
            NspirePath element = absolute.getName(i);
                ...
                (element.asByteArray()[1] == '.'))
            {
                java.nio.file.attribute.BasicFileAttributes attrs =
                    Files.readAttributes(result, java.nio.file.attribute.BasicFileAttributes.class,
        ...
        // Check existence without resolving the final link.
        Files.readAttributes(result, java.nio.file.attribute.BasicFileAttributes.class,
                             LinkOption.NOFOLLOW_LINKS);
        return result;
    }
    ...
    @Override
    public URI toUri() {
        return toAbsolutePath().toFile().toURI();
    }
        ...
        throws IOException
    {
        if (watcher == null)
            throw new NullPointerException();
        throw new UnsupportedOperationException("Ndless watch service is not implemented");
    }
}
